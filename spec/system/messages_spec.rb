...
RSpec.describe "メッセージ投稿機能", type: :system do
  ...
  context '投稿に成功したとき' do
    ...
    it '画像の投稿に成功すると、投稿一覧に遷移して、投稿した画像が表示されている' do
      ...
      # 送信した値がDBに保存されていることを期待する
      expect{
        find('input[name="commit"]').click #例 ('input[name="commit"]')
      }.to change { Message.count }.by(1) #例{"クリックしたら"}

      # 投稿一覧画面に遷移することを期待する
      expect(current_path).to eq room_messages_path(@room_user.room) #Prefixはrails routesで確認

      # 送信した画像がブラウザに表示されていることを期待する
      expect(page).to have_selector("img") #have_selector".要素名[ '(#{画像のURL});']"
    end
